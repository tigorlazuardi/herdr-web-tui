...
Always redact VAPID private key, push endpoint/auth/p256dh, cookies/headers as `<redacted>`. `Remote-User` support-debug handle may remain visible but never metric label. Agent name allowed only in push payload; telemetry uses `agent.name=<redacted>`. Pane ID allowed only in push/click/focus request payloads; telemetry uses `pane.id=<redacted>` and never pane ID metric labels. Local slog output remains active regardless of exporter availability.
